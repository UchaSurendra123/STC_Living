  ...
  try {
    ...
    if (process.env.SMTP_USER && process.env.SMTP_PASS) {
      try {
        ...
        console.error('❌ Email notification failed:', mailErr);
        // Throw error in production so Vercel logs reveal the exact SMTP issue
        if (process.env.VERCEL) {
          throw new Error(`Email delivery failed: ${mailErr.message}`);
        }
      }
    } else {
      console.warn('⚠️ SMTP credentials (SMTP_USER/SMTP_PASS) are missing in current environment!');
    }
