...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      // Normalize form payload structure to guarantee match with backend expectations
      const payload = {
        name: rawData.name || rawData.contactName || rawData.fullName || '',
        email: rawData.email || '',
        phone: rawData.phone || rawData.mobile || '',
        propertyName: rawData.propertyName || 'STC Living',
      ...
      try {
        ...
        console.error('Form submission error:', err);
        if (status) {
          status.textContent = err.message || 'Unable to send enquiry. Please try again later or contact us directly.';
          status.className = 'form-status error';
        }
      } finally {
        if (btn) {
